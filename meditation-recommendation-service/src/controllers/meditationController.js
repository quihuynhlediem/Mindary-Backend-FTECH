...
const getAllMeditations = async (req, res) => {
	try {
		...
		return res.status(500).json({
			message: "An unexpected error occurred while fetching meditations.",
		});
	}
};
...
const getRecommendations = async (req, res) => {
	try {
		...
		if (typeof userId !== "string" || typeof diaryId !== "string") {
			return res
				.status(400)
				.json({ message: "User ID and date must be strings." });
		}

		const recommendations = await MeditationService.getRecommendations(
			userId,
			diaryId
		);
		if (!recommendations) {
			return res
				.status(404)
				.json({ message: "Recommended meditation not found" });
		}
